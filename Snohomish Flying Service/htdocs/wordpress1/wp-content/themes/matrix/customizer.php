...
// Change control title and description.
    $wp_customize->get_section('background_image')->title       = esc_html__('Site Layout & Background', 'matrix');
    $wp_customize->get_section('background_image')->description = esc_html__('Site Layout & Background Options', 'matrix');

    $wp_customize->get_section('title_tagline')->title = esc_html__('Site Logo/Title/Tagline', 'matrix');
// Change header image section title & priority.
    $wp_customize->get_section('header_image')->title = esc_html__('Header Options', 'matrix');

// move general control into general panel
    $wp_customize->get_section('static_front_page')->panel = 'matrix_general_settings_panel';
    $wp_customize->get_section('title_tagline')->panel     = 'matrix_general_settings_panel';
    $wp_customize->get_section('background_image')->panel  = 'matrix_general_settings_panel';
    $wp_customize->get_section('header_image')->panel      = 'matrix_general_settings_panel';
// Selective refresh.
...
{
    $matrix_theme_options = matrix_theme_options();
    echo '<h1 id="service-head">' . wp_kses_post($matrix_theme_options['home_service_title']) . '</h1>';
}
function matrix_partial_refresh_service_desc()
{
    $matrix_theme_options = matrix_theme_options();
    echo '<p id="service-desc">' . wp_kses_post($matrix_theme_options['home_service_description']) . '</p>';
}
function matrix_partial_refresh_service_icon($i)
{
    ...
    $out .= '<i id="service-icon-' . esc_attr($i) . '" class="fa fa-' . esc_attr($matrix_theme_options['service_icon_' . $i]) . ' icon-large"></i>';
    $out .= '</div>';
    echo $out;
}
...
{
    $matrix_theme_options = matrix_theme_options();
    echo $out             = '<h4 id="service-title-' . esc_attr($i) . '" >' . wp_kses_post($matrix_theme_options['service_title_' . $i]) . '</h4>';
}
function matrix_partial_refresh_service_text($i)
{
    $matrix_theme_options = matrix_theme_options();
    echo $out             = '<p id="service-desc-' . esc_attr($i) . '" >' . wp_kses_post($matrix_theme_options['service_text_' . $i]) . '</p>';
}
/* portfolio partial refresh */
function matrix_partial_refresh_portfolio_head()
{
    $matrix_theme_options = matrix_theme_options();
    echo '<h1 id="portfolio-head">' . wp_kses_post($matrix_theme_options['home_portfolio_title']) . '</h1>';
}
function matrix_partial_refresh_portfolio_desc()
{
    $matrix_theme_options = matrix_theme_options();
    echo '<p id="portfolio-desc" class="text-center">' . wp_kses_post($matrix_theme_options['home_portfolio_description']) . '</p>';
}
/***************************/
...
{
    $matrix_theme_options = matrix_theme_options();
    echo $out             = '<h1 id="blog-heading" >' . wp_kses_post($matrix_theme_options['home_blog_title']) . '</h1>';
}
function matrix_partial_refresh_blog_desc()
{
    $matrix_theme_options = matrix_theme_options();
    echo $out             = '<p id="blog-desc" class="text-center">' . wp_kses_post($matrix_theme_options['home_blog_description']) . '</p>';
}
function matrix_partial_refresh_extra_title()
{
    $matrix_theme_options = matrix_theme_options();
    echo $out             = '<h1 id="extra_title" >' . wp_kses_post($matrix_theme_options['home_extra_title']) . '</h1>';
}function matrix_partial_refresh_extra_desc()
{
    $matrix_theme_options = matrix_theme_options();
    echo $out             = '<p id="extra_desc" class="text-center">' . wp_kses_post($matrix_theme_options['home_extra_description']) . '</p>';
}
function matrix_partial_refresh_extra_content()
{
    $matrix_theme_options = matrix_theme_options();
    echo '<div class="latest-posts">' . wp_kses_post($matrix_theme_options['extra_content_home']) . '</div>';
}
...
Kirki::add_panel('matrix_general_settings_panel', array(
    'priority' => 10,
    'title'    => __('General Settings', 'matrix'),
));
Kirki::add_panel('matrix_option_panel', array(
    'priority'    => 20,
    'title'       => __('Home Sections', 'matrix'),
    'description' => __('Here you can customize all your site contents', 'matrix'),
));
/* Add control to the topbar section of customizer */
Kirki::add_section('topbar_sec', array(
    'title'      => __('Top bar options', 'matrix'),
    'panel'      => 'matrix_general_settings_panel',
    'priority'   => 20,
    'capability' => 'edit_theme_options',
));
Kirki::add_field('matrix_theme', array(
    'settings'          => 'show_topbar',
    'label'             => __('Show Topbar', 'matrix'),
    'section'           => 'topbar_sec',
    'type'              => 'checkbox',
    'priority'          => 9,
    'default'           => 1,
...
Kirki::add_field('matrix_theme', array(
    ...
    'label'             => __('Background Color', 'matrix'),
...
/* Slider title, subtitle typography */
Kirki::add_field('matrix_theme', array(
    'type'        => 'typography',
    'settings'    => 'slider_title_font',
    'label'       => __('Slider Title & Subtitle Typography', 'matrix'),
    'section'     => 'typography_sec',
    'default'     => array(
        'font-family' => "Open Sans",
    ),
    ...
    'output'      => array(
        array(
            'element' => '.slider-content .text-center span',
        ),
    ),
));
...
Kirki::add_field('matrix_theme', array(
    ...
    'label'       => __('Heading Typography', 'matrix'),
    'description' => __('Change font style of home of heading i.e. h1, h2, h3, ... h6', 'matrix'),
    'section'     => 'typography_sec',
    'default'     => array(
        'font-style'  => array('bold', 'italic'),
        'font-family' => "Open Sans",
...
Kirki::add_field('matrix_theme', array(
    ...
    'output'      => array(
        array(
            'element' => 'body, p, em, blockquote, .main_title h2',
        ),
    ),
));
/* Slider */
...
Kirki::add_field('matrix_theme', array(
    'type'     => 'custom',
    'settings' => 'link_to_contrl',
    'section'  => 'slider_sec',
    'default'  => sprintf(__('You can enable/disable and re-arrange slider from %1$s Home Page Re-order Section.%2$s', 'matrix'), '<a href="javascript:wp.customize.section( \'home_customize_section\' ).focus();">', '</a>'),
    'priority' => 10,
));
...
for ($i = 1; $i <= 3; $i++) {
    Kirki::add_field('matrix_theme', array(
        ...
        'label'             => sprintf(__('Slider %s', 'matrix'), $num[$i]),
        'section'           => 'slider_sec',
        'type'              => 'heading',
        'priority'          => 10,
        'sanitize_callback' => 'matrix_sanitize_text',
    ...
    Kirki::add_field('matrix_theme', array(
        ...
        'label'       => sprintf(__('Slider %s Image', 'matrix'), $num[$i]),
        'description' => __('Recommended image size is 1440x500px', 'matrix'),
        'type'        => 'image',
        'section'     => 'slider_sec',
        'priority'    => 10,
        'default'     => get_template_directory_uri() . '/images/slide.jpg',
    ));
    Kirki::add_field('matrix_theme', array(
        'settings'          => 'slider_title_' . $i,
        'label'             => sprintf(__('Slider %s Title', 'matrix'), $num[$i]),
        'type'              => 'text',
        'default'           => $matrix_theme_options['slider_title_' . $i],
        'section'           => 'slider_sec',
        'priority'          => 10,
        'sanitize_callback' => 'matrix_sanitize_text',
    ));
    Kirki::add_field('matrix_theme', array(
        ...
        'label'             => sprintf(__('Slider %s Subitle', 'matrix'), $num[$i]),
        'type'              => 'text',
        'default'           => $matrix_theme_options['slider_subtitle_' . $i],
        'section'           => 'slider_sec',
        'priority'          => 10,
        'sanitize_callback' => 'matrix_sanitize_text',
    ));
    Kirki::add_field('matrix_theme', array(
        ...
        'label'    => __('Link this slide to a post or page', 'matrix'),
        'section'  => 'slider_sec',
        'default'  => 'post',
        'priority' => 10,
        'choices'  => array(
            'post' => esc_attr__('Post', 'matrix'),
            'page' => esc_attr__('Page', 'matrix'),
        ),
    ));
    ...
    Kirki::add_field('matrix_theme', array(
        ...
        'label'             => __('Read more button text', 'matrix'),
        'type'              => 'text',
        'section'           => 'slider_sec',
        'priority'          => 10,
        'default'           => __('Read More', 'matrix'),
        'sanitize_callback' => 'matrix_sanitize_text',
        'active_callback'   => function () use ($i) {
            $matrix_options = get_option('matrix_theme_options');
            if (isset($matrix_options['slider_post_page_' . $i]) && $matrix_options['slider_post_page_' . $i] == 'post') {
                if (isset($matrix_options['post_slider_' . $i]) && $matrix_options['post_slider_' . $i] != 0) {return true;}
            } else {
                if (isset($matrix_options['page_slider_' . $i]) && $matrix_options['page_slider_' . $i] != 0) {return true;}
            }
            return false;
        },
    ));
...
Kirki::add_field('matrix_theme', array(
    ...
    'label'    => __('Plugin Shortcode', 'matrix'),
    'section'  => 'slider_sec',
    'priority' => 10,
...
Kirki::add_section('service_sec', array(
    ...
    'description' => __('You can use basic html tag like <p>, <strong>, <span>, <a> etc in Text and Textarea field', 'matrix'),
    'panel'       => 'matrix_option_panel',
    'priority'    => 160,
    'capability'  => 'edit_theme_options',
));
Kirki::add_field( 'matrix_theme', array(
    'type'        => 'switch',
    'settings'    => 'home_service_boxed',
    'label'       => __( 'Service Boxed Layout', 'matrix' ),
    'section'     => 'service_sec',
    'default'     => 'red',
    'priority'    => 10,
    'choices'     => array(
        'on'   => esc_attr__( 'Yes', 'matrix' ),
        'off' => esc_attr__( 'No', 'matrix' ),
    ),
) );
Kirki::add_field('matrix_theme', array(
    'settings'          => 'home_service_hover_effect',
    'label'             => __('Hover Zoom Effect', 'matrix'),
    'section'           => 'service_sec',
    'type'              => 'switch',
    'priority'          => 10,
    'default'           => 0,
...
Kirki::add_field('matrix_theme', array(
    ...
    'section'           => 'service_sec',
    'type'              => 'text',
    'description'       => sprintf(__('Wrap any word(s) into span tag to make it\'s color match with theme color scheme.<br> e.g. %1$s Our &lt;span&gt;Services&lt;/span&gt; %2$s','matrix'), '<span style="color:#333">', '</span>'),
    'priority'          => 10,
    'transport'         => 'postMessage',
...
for ($i = 1; $i <= 4; $i++) {
    Kirki::add_field('matrix_theme', array(
        ...
        'label'             => sprintf(__('Service %s', 'matrix'), $num[$i]),
        'section'           => 'service_sec',
        'type'              => 'heading',
        'priority'          => 10,
        'sanitize_callback' => 'matrix_sanitize_text',
...
Kirki::add_field('matrix_theme', array(
    ...
    'type'              => 'text',
    'description'       => sprintf(__('Wrap any word(s) into span tag to make it\'s color match with theme color scheme.<br> e.g. %1$s Our &lt;span&gt;Portfolio&lt;/span&gt; %2$s','matrix'), '<span style="color:#333">', '</span>'),
    'priority'          => 10,
    'transport'         => 'postMessage',
    'default'           => $matrix_theme_options['home_portfolio_title'],
    'sanitize_callback' => 'matrix_sanitize_text',
    'partial_refresh'   => array(
        'home_portfolio_title' => array(
            'selector'            => '#portfolio-head',
            'container_inclusive' => true,
            'render_callback'     => 'matrix_partial_refresh_portfolio_head',
        ),
    ),
));
Kirki::add_field('matrix_theme', array(
    ...
    'priority'          => 10,
    'transport'         => 'postMessage',
    'default'           => $matrix_theme_options['home_portfolio_description'],
    'sanitize_callback' => 'matrix_sanitize_text',
    'partial_refresh'   => array(
        'home_portfolio_description' => array(
            'selector'            => '#portfolio-desc',
            'container_inclusive' => true,
            'render_callback'     => 'matrix_partial_refresh_portfolio_desc',
        ),
    ),
));
Kirki::add_field('matrix_theme', array(
    'settings'          => 'portfolio_gallery',
    'label'             => __('Create Portfolio Gallery', 'matrix'),
    'section'           => 'portfolio_sec',
    'type'              => 'gallery',
    'priority'          => 10,
    'sanitize_callback' => 'matrix_sanitize_text',
...
Kirki::add_field('matrix_theme', array(
    'settings'          => 'portfolio_shortcode_heading',
    'label'             => __('Display Portfolio using Plugin', 'matrix'),
    'section'           => 'portfolio_sec',
    'type'              => 'heading',
    'priority'          => 10,
    'sanitize_callback' => 'matrix_sanitize_text',
...
Kirki::add_field('matrix_theme', array(
    ...
    'description'             => __('This option will work only for Photo Video Gallery Master plugin', 'matrix'),
    'tooltip'              => __('This option work with only "Photo Video Gallery Master" plugin', 'matrix'),
    'section'           => 'portfolio_sec',
    'type'              => 'checkbox',
    'priority'          => 10,
    'transport'         => 'postMessage',
...
Kirki::add_field('matrix_theme', array(
    'settings'    => 'home_post_num',
    'label'       => __('Number f posts you want to show on home page.', 'matrix'),
    'section'     => 'blog_sec',
    'type'        => 'number',
    'priority'    => 10,
    'default'     => 3,
...
Kirki::add_field('matrix_theme', array(
    ...
    'label'       => __('Show Load More Button.', 'matrix'),
    'section'     => 'blog_sec',
    'type'        => 'switch',
    'priority'    => 10,
    'default'     => 1,
...
Kirki::add_field('matrix_theme', array(
    ...
    'label'       => __('Number of load more posts.', 'matrix'),
    'description'        => __('Number of posts you want to load on "Load More" button click', 'matrix'),
    'section'     => 'blog_sec',
    'type'        => 'number',
    'priority'    => 10,
    'default'     => 3,
...
Kirki::add_field('matrix_theme', array(
    'settings'          => 'blog_string',
    'label'             => __('Strings Used in Home Blog section', 'matrix'),
    'section'           => 'blog_sec',
    'type'              => 'heading',
    'priority'          => 10,
    'sanitize_callback' => 'matrix_sanitize_text',
...
Kirki::add_field('matrix_theme', array(
    ...
    'label'             => __('Load More Text', 'matrix'),
    'section'           => 'blog_sec',
    'type'              => 'text',
    'default'           =>$matrix_theme_options['blog_load_more_text'],
    'priority'          => 10,
    'sanitize_callback' => 'matrix_sanitize_text',
));
Kirki::add_field('matrix_theme', array(
    'settings'          => 'blog_more_loading',
    'label'             => __('Loading Text', 'matrix'),
    'section'           => 'blog_sec',
    'type'              => 'text',
    'default'           =>$matrix_theme_options['blog_more_loading'],
    'priority'          => 10,
    'sanitize_callback' => 'matrix_sanitize_text',
));
Kirki::add_field('matrix_theme', array(
    'settings'          => 'blog_no_more_post',
    'label'             => __('No more post Text', 'matrix'),
    'section'           => 'blog_sec',
    'type'              => 'text',
    'default'           =>$matrix_theme_options['blog_no_more_post'],
    'priority'          => 10,
    'sanitize_callback' => 'matrix_sanitize_text',
));
/* Social Options */
...
Kirki::add_field('matrix_theme', array(
    ...
    'label'             => __('Skype', 'matrix'),
    'description'             => __('e.g. skype:SKYPE_USERNAME?call', 'matrix'),
    'section'           => 'social_sec',
    'type'              => 'text',
    'priority'          => 10,
    'transport'         => 'postMessage',
...
function matrix_controls_enqueue_scripts($wp_customize)
{

    wp_enqueue_style('customizercustom_css', get_template_directory_uri() . '/css/customizer.css');
    wp_enqueue_script('matrix_customize_control', get_template_directory_uri() . '/js/matrix_customize_control.js', array('jquery', 'customize-controls'));

}
add_action('customize_controls_enqueue_scripts', 'matrix_controls_enqueue_scripts', 11);
...
add_action('customize_register', function ($wp_customize) {
    class Matrix_Customize_Heading extends WP_Customize_Control
    {
        public $type = 'heading';
        ...
        {
            if (!empty($this->label)): ?>
                <h3 class="matrix-accordion-section-title"><?php echo esc_html($this->label); ?></h3>
            <?php endif;
    ...
    /* gallery field */
    class Matrix_Display_Gallery_Control extends WP_Customize_Control{
        public $type = 'gallery';
        ...
        <label>
            ...
            <input id="edit-gallery" class="button upload_gallery_button" type="button" value="<?php _e('Add/Edit Gallery','matrix') ?>" />
            <input id="clear-gallery" class="button upload_gallery_button" type="button" value="<?php _e('Clear','matrix') ?>" />
    ...
    add_filter('kirki/control_types', function ($controls) {
        $controls['heading'] = 'Matrix_Customize_Heading';
         $controls['gallery'] = 'Matrix_Display_Gallery_Control';
        return $controls;
    });
});
?>