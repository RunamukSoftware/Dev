...
function kad_image_menu_shortcode_function( $atts, $content) {
    ...
    if ($columns == '2') {
   	...
   	<?php if($height_setting == 'imgsize') { 
						...
						if(empty($the_image[0])) {$the_image[0] = $image; $the_image[1] = null; $the_image[2] = null;} ?>
							<div class="<?php echo esc_attr($itemsize);?> image-menu-class-id-<?php echo esc_attr($id);?> kad-animation image-menu-image-size" data-animation="fade-in" data-delay="150">
	...
	<?php } else { 
			...
			if(empty($the_image)) {$the_image = $image;}?>
		        <div class="<?php echo esc_attr($itemsize);?> image-menu-class-id-<?php echo esc_attr($id);?> kad-animation" data-animation="fade-in" data-delay="150">
