...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:udt="DotNetNuke/UserDefinedTable" exclude-result-prefixes="udt">
  ...
  <xsl:template match="udt:Data" mode="list">
    <tr class="dnnGridItem">
      ...
      <td>
        <a href="{udt:Link_x0020_URL}" target="_blank">
          <xsl:value-of select="udt:Category" disable-output-escaping="yes" /> - <xsl:value-of select="udt:Title" disable-output-escaping="yes" /></a>
      </td>
      <td>
        <xsl:value-of select="udt:Notes" disable-output-escaping="yes" />
      </td>
    </tr>
  </xsl:template>
  ...
  <xsl:template match="/udt:UserDefinedTable">
    ...
    <xsl:if test="$currentData">
      <table class="dnnFormItem">
        ...
        Category
          <xsl:apply-templates select="$currentData" mode="list"></xsl:apply-templates></table>
    </xsl:if>
  </xsl:template>
